...
(() => {
 ...
 let currentUser=null, stays=[], settings=[], editing=null;
 const fields=["no_label","sort_order","name","slug","label","capacity","room_types","address","checkin","checkout","security_deposit","booking_deposit","extra_bed","facilities","high_season_price","low_season_price","note","cover_image_url","is_published"];
 function toast(msg){const t=$("toast");t.textContent=msg;t.classList.add("show");setTimeout(()=>t.classList.remove("show"),2400)}
 function safeName(name){return name.toLowerCase().replace(/[^a-z0-9._-]/g,"-")}
...
$("authForm").addEventListener("submit", async (e) => {
 ...
 $("signupBtn").addEventListener("click",async()=>{
   ...
   if(!email||password.length<8){alert("請輸入 Email，密碼至少 8 碼。");return}
   const {data,error}=await db.auth.signUp({email,password});
   if(error) return alert(error.message);
   alert(data.session?"註冊完成，已登入。":"註冊完成。若 Supabase 有開啟 Email 驗證，請先至信箱確認。");
 });
 $("logoutBtn").addEventListener("click",()=>db.auth.signOut());
 db.auth.onAuthStateChange((_e,session)=>setTimeout(()=>showSession(session),0));
 db.auth.getSession().then(({data})=>showSession(data.session));

 async function loadStays(){
   const {data,error}=await db.from("stays").select("*,stay_images(*)").order("sort_order");
   if(error)return alert(error.message);stays=data||[];renderStays();
 }
...
function initStaySortable() {
 ...
 $("stayList").addEventListener("click",e=>{const b=e.target.closest("[data-edit]");if(b)openEdit(stays.find(s=>s.id===b.dataset.edit))});
 $("newStayBtn").addEventListener("click",()=>openEdit(null));
 function openEdit(s){
   editing=s||null;$("editTitle").textContent=s?"編輯民宿":"新增民宿";$("stayForm").reset();$("stayId").value=s?.id||"";
   fields.forEach(f=>{const el=$(f);if(!el)return;if(el.type==="checkbox")el.checked=s?!!s[f]:true;else el.value=s?.[f]??(f==="sort_order"?stays.length+1:"")});
   $("deleteStayBtn").style.visibility=s?"visible":"hidden";$("galleryManager").hidden=!s;
   if(s)renderGallery(s);$("editModal").hidden=false;
 }
 function closeEdit(){$("editModal").hidden=true;editing=null}
 document.addEventListener("click",e=>{if(e.target.closest("[data-close-edit]"))closeEdit()});
 async function uploadFile(file,folder){
   const path=`${folder}/${Date.now()}-${safeName(file.name)}`;
   const {error}=await db.storage.from("homestay-images").upload(path,file,{upsert:false});
 ...
 $("stayForm").addEventListener("submit",async e=>{
   e.preventDefault();
   try{
     const row={};fields.forEach(f=>{const el=$(f);row[f]=el.type==="checkbox"?el.checked:(f==="sort_order"?Number(el.value):el.value.trim())});
     const file=$("coverFile").files[0];if(file)row.cover_image_url=await uploadFile(file,`covers/${row.slug||"stay"}`);
     let result;
     if(editing)result=await db.from("stays").update(row).eq("id",editing.id).select().single();
     else result=await db.from("stays").insert(row).select().single();
     if(result.error)throw result.error;toast("民宿已儲存");closeEdit();await loadStays();
   }catch(err){alert(err.message)}
 });
 $("deleteStayBtn").addEventListener("click",async()=>{
   if(!editing||!confirm(`確定刪除「${editing.name}」？相簿資料也會刪除。`))return;
   const {error}=await db.from("stays").delete().eq("id",editing.id);if(error)return alert(error.message);
   toast("民宿已刪除");closeEdit();await loadStays();
 });
 function renderGallery(s){
   const imgs=(s.stay_images||[]).sort((a,b)=>a.sort_order-b.sort_order);
   $("adminGallery").innerHTML=imgs.map(x=>`<div class="admin-photo"><img src="${x.image_url}"><button data-del-img="${x.id}" title="刪除">×</button><small>${x.category}</small></div>`).join("")||"<p>尚無相簿照片。</p>";
 }
 $("adminGallery").addEventListener("click",async e=>{
   const b=e.target.closest("[data-del-img]");if(!b||!confirm("確定刪除這張照片？"))return;
   const {error}=await db.from("stay_images").delete().eq("id",b.dataset.delImg);if(error)return alert(error.message);
   toast("照片已刪除");await loadStays();editing=stays.find(s=>s.id===editing.id);renderGallery(editing);
 });
 $("uploadGalleryBtn").addEventListener("click",async()=>{
   if(!editing)return;const files=[...$("galleryFiles").files];if(!files.length)return alert("請先選擇照片。");
   const category=$("imageCategory").value;
   try{
     const rows=[];
     for(let i=0;i<files.length;i++){const url=await uploadFile(files[i],`galleries/${editing.slug}/${category}`);rows.push({stay_id:editing.id,category,image_url:url,sort_order:(editing.stay_images?.length||0)+i+1})}
     const {error}=await db.from("stay_images").insert(rows);if(error)throw error;
     $("galleryFiles").value="";toast("照片已上傳");await loadStays();editing=stays.find(s=>s.id===editing.id);renderGallery(editing);
   }catch(err){alert(err.message)}
 });
 ...
 async function loadSettings(){
   const {data,error}=await db.from("site_settings").select("*").order("key");if(error)return alert(error.message);settings=data||[];
   $("settingsForm").innerHTML=Object.entries(settingLabels).map(([key,label])=>{const value=settings.find(x=>x.key===key)?.value||"";const multi=key.includes("_p")||key.includes("note")||key.includes("body")||key==="contact_text";return `<label class="${multi?"wide":""}">${label}${multi?`<textarea data-setting="${key}" rows="3">${value}</textarea>`:`<input data-setting="${key}" value="${String(value).replace(/"/g,"&quot;")}">`}</label>`}).join("");
    const bannerUrl =
 ...
 $("saveSettingsBtn").addEventListener("click",async()=>{
   const rows=[...document.querySelectorAll("[data-setting]")].map(el=>({key:el.dataset.setting,value:el.value}));
   const {error}=await db.from("site_settings").upsert(rows,{onConflict:"key"});if(error)return alert(error.message);toast("網站內容已儲存");
 });
 const tabSections = {
  stays: "staysTab",
...
document.querySelectorAll(".tab").forEach(button => {
 ...
 let priceNotes = [];
  ...
  function renderPriceNotes() {
    $("priceNoteList").innerHTML = priceNotes.map(note => `
      <article
        class="dynamic-item"
...
function initPriceNoteSortable() {
  ...
  $("priceNoteList").addEventListener("click", async event => {
    ...
    if (saveButton) {
      ...
      const content =
        document.querySelector(`[data-price-content="${id}"]`).value;

      const sortOrder =
        Number(document.querySelector(`[data-price-order="${id}"]`).value);

      const isVisible =
        document.querySelector(`[data-price-visible="${id}"]`).checked;
...
function initBookingNoticeSortable() {
  ...
  $("bookingNoticeList").addEventListener("click", async event => {
    ...
    if (saveButton) {
      ...
      const title =
        document.querySelector(`[data-notice-title="${id}"]`).value;

      const content =
        document.querySelector(`[data-notice-content="${id}"]`).value;

      const sortOrder =
        Number(document.querySelector(`[data-notice-order="${id}"]`).value);

      const isVisible =
        document.querySelector(`[data-notice-visible="${id}"]`).checked;
...
$("contactLinkList").addEventListener("click", async event => {
  const saveButton =
    event.target.closest("[data-save-contact]");

  const deleteButton =
    event.target.closest("[data-delete-contact]");
  ...
  if (saveButton) {
    ...
    const type =
      document.querySelector(
        `[data-contact-type="${id}"]`
      ).value;

    const title =
      document.querySelector(
        `[data-contact-title="${id}"]`
      ).value.trim();

    const value =
      document.querySelector(
        `[data-contact-value="${id}"]`
      ).value.trim();

    const sortOrder =
      Number(
        document.querySelector(
          `[data-contact-order="${id}"]`
        ).value
      );
